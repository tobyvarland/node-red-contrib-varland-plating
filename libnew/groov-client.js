...
/** @class GroovClient used as a consumer of the PAC REST API. */
class GroovClient {
   ...
   * @returns {(number|boolean|null)} Variable value or null on error.
   */
  getIntegerTableIndex(name, index) {
    try {
   ...
   * @returns {(string|null)} Variable value or null on error.
   */
  getStringTableIndex(name, index) {
    try {
   ...
   * @returns {boolean} True on success, false on error.
   */
  setIntegerTable(name, value, startIndex = 0) {
    try {
   ...
   * @returns {boolean} True on success, false on error.
   */
  setFloatTable(name, value, startIndex = 0) {
    try {
   ...
   * @returns {boolean} True on success, false on error.
   */
  setStringTable(name, value, startIndex = 0) {
    try {
   ...
   * @returns {boolean} True on success, false on error.
   */
  setIntegerTableIndex(name, index, value) {
    try {
      if (typeof value == "boolean") value = (value ? 1 : 0)
   ...
   * @returns {boolean} True on success, false on error.
   */
  setFloatTableIndex(name, index, value) {
    try {
   ...
   * @returns {boolean} True on success, false on error.
   */
  setStringTableIndex(name, index, value) {
    try {
