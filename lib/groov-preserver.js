 ...
 * @property {string} SAVE_DELAY_VARIABLE Name of timer to indicate save process is delayed.
 * @property {string} STATUS_VARIABLE Name of recipe system status variable.
 */
GroovPreserver.FINISHED_VARIABLE = "b_Recipes_Finished";
...
GroovPreserver.SAVE_DELAY_VARIABLE = "hdt_Recipes_DelaySave";
...
GroovPreserver.prototype.process = function() {
  if (this.groov.getInteger(GroovPreserver.STATUS_VARIABLE) != 0) return;
  if (this.groov.getInteger(GroovPreserver.FINISHED_VARIABLE)) {
    if (this.groov.getDownTimer(GroovPreserver.SAVE_DELAY_VARIABLE) == 0) {
      this.saveData();
    }
  } else {
    this.restoreData();
...
GroovPreserver.prototype.restoreData = function() {
  this.groov.setInteger(GroovPreserver.STATUS_VARIABLE, 1);
  ...
  if (this.missingVariables.length > 0) {
    ...
    this.groov.setVariable(GroovPreserver.FINISHED_VARIABLE, true);
  }
}
...
GroovPreserver.prototype.saveData = function() {
  this.groov.setInteger(GroovPreserver.STATUS_VARIABLE, 2);
  ...
  this.groov.setInteger(GroovPreserver.STATUS_VARIABLE, 3);
}
